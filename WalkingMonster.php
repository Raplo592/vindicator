...
abstract class WalkingMonster extends WalkingEntity implements Monster{
    ...
    public function entityBaseTick($tickDiff = 1): bool{
        ...
        if(!$this->hasEffect(Effect::WATER_BREATHING) && $this->isInsideOfWater()){
            $hasUpdate = true;
            $airTicks = $this->propertyManager->getPropertyType(self::DATA_AIR) - $tickDiff;
            if($airTicks <= -20){
                $airTicks = 0;
                $ev = new EntityDamageEvent($this, EntityDamageEvent::CAUSE_DROWNING, 2);
                $this->attack($ev);
            }
            $this->propertyManager->setPropertyValue(self::DATA_AIR, self::DATA_TYPE_SHORT, $airTicks);
        }else{
            $this->propertyManager->setPropertyValue(self::DATA_AIR, self::DATA_TYPE_SHORT, 300);
        }
